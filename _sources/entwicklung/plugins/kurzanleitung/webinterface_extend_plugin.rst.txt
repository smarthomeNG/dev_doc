...
Die Methode ``index`` der Klasse ``WebInterface`` implementiert die Seitenerstellung des Webinterfaces. Es rendert das
Template (``webif/templates/index.html``) und macht das Plugin für den Renderer als ``p`` verfügbar
(``return tmpl.render(p=self.plugin)``). Darüber hinaus erforderliche Daten können dem Aufruf von ``tmpl.render()``
